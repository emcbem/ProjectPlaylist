...
function Dropdown({searchRequest, setSearchRequest}: {searchRequest: GetGamesRequest, setSearchRequest: React.Dispatch<React.SetStateAction<GetGamesRequest>>}) {
  ...
  return (
    <div className="relative inline-block text-left">
      <button
        ...
        className="inline-flex justify-between items-center w-full px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
      >
        Sort: {searchRequest.orderingMethod}
      ...
      {isOpen && (
        <div className="absolute right-0 z-10 w-full mt-2 origin-top-right bg-white border border-gray-300 rounded-md shadow-lg">
          <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
            ...
            <button onClick={() =>handleClick(OrderingMethods.MostPlayed)} className="block w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">Most Played</button>
          </div>
        </div>
      )}
    </div>
