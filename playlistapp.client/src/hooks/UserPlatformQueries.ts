...
export const UserPlatformQueries = {
    ...
    UpdateUserPlatform: () => {
        ...
        return useMutation({
            mutationFn: (request: UpdateUserPlatformRequest) => UserPlatformService.UpdateUserPlatform(request),
            onSuccess: () => {
                queryClient.invalidateQueries({ queryKey: UserPlatformKeys.GetByUserKey })
            }
        })
    },
    AddUserPlatform: () => {
        ...
        return useMutation({
            mutationFn: (request: AddUserPlatformRequest) => UserPlatformService.AddUserPlatform(request),
            onSuccess: () => {
                queryClient.invalidateQueries({ queryKey: UserPlatformKeys.GetByUserKey })
            }
        })
    ...
    DeleteUserPlatform: () => {
        ...
        return useMutation({
            mutationFn: (userPlatformId: number) => UserPlatformService.DeleteUserPlatform(userPlatformId),
            onSuccess: () => {
                queryClient.invalidateQueries({ queryKey: UserPlatformKeys.GetByUserKey })
            }
        })
