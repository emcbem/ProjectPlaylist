...
const MasonryCards: React.FC<props> = ({ games }) => {
  ...
  const cards = games.map((game) => {
    ...
    return (
        ...
        <div className="rounded-lg overflow-hidden dark:text-white text-black">
          <img
            src={game.platformGame.game.coverUrl}
            className={`w-full sm:h-[${randomHeight}px] h-[${
              randomHeight / 2
            }px] object-cover`}
          />
          <div className="dark:bg-[#252A2C] bg-[#D9D9D9] hidden">
            ...
            <div className="sm:text-2xl text-base px-2 pt-2 font-extrabold">
              {game.platformGame.game.title}
            </div>
            <div className="sm:text-base text-xs px-2 pt-2 ">
              Rating: {game.platformGame.game.ageRating}
            </div>
            <div className="sm:text-base text-xs px-2 py-2 ">
              Release:{" "}
              {game.platformGame.game.publishDate
                ? new Date(game.platformGame.game.publishDate).toLocaleDateString(
                    undefined,
                    options
                  )
                : ""}
